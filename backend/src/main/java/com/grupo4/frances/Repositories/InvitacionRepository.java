...
public interface InvitacionRepository extends JpaRepository<Invitacion, Long> {
    ...
    List<Invitacion> buscarInvitacionesPendientes(@Param("idAlumno") Long idAlumno);

    @Query("SELECT i FROM Invitacion i WHERE i.idGrupo = :idGrupo AND i.idAlumno = :idAlumno")
    Invitacion buscarPorGrupoYAlumno(@Param("idGrupo") Long idGrupo, @Param("idAlumno") Long idAlumno);

    @Query("SELECT i FROM Invitacion i WHERE i.idProfesor = :idProfesor")
    List<Invitacion> buscarPorProfesor(@Param("idProfesor") Long idProfesor);
